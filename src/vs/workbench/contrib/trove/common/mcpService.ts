...
// This service implements the Model Context Protocol (MCP) integration,
// enabling the AI agent to communicate with external tool servers.
...
import { MCPUserStateOfName } from './troveSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('trove-channel-mcp')
	...
	private async _initialize() {
		try {
			await this.troveSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.troveSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.troveSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.troveSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		await this.troveSettingsService.setMCPServerState(serverName, { isOn });
