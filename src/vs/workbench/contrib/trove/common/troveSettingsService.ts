...
import { TROVE_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, TroveStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './troveSettingsTypes.js';
...
export type TroveSettingsState = {
...
// type RealVoidSettings = Exclude<keyof TroveSettingsState, '_modelOptions'>
// type EventProp<T extends RealVoidSettings = RealVoidSettings> = T extends 'globalSettings' ? [T, keyof TroveSettingsState[T]] : T | 'all'


export interface ITroveSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: TroveSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: TroveSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: TroveStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	...
	return [
		...
		...existingModels.filter(m => {
			// drop replaced defaults and any custom/autodetected entry with the same name as an incoming default
			if (m.type === type) return false
			if (type === 'default' && incomingModelNames.has(m.modelName)) return false
			return true
		})
	]
}
...
const _validatedModelState = (state: Omit<TroveSettingsState, '_modelOptions'>): TroveSettingsState => {
	...
	for (const providerName of providerNames) {
		...
		for (const { modelName, isHidden } of newSettingsOfProvider[providerName].models) {
			...
			if (seenModelSelections.has(dedupeKey)) continue
			seenModelSelections.add(dedupeKey)
...
export const ITroveSettingsService = createDecorator<ITroveSettingsService>('TroveSettingsService');
class TroveSettingsService extends Disposable implements ITroveSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: TroveSettingsState;
	...
	dangerousSetState = async (newState: TroveSettingsState) => {
	...
	private async _readState(): Promise<TroveSettingsState> {
		let encryptedState = this._storageService.get(TROVE_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
		if (!encryptedState) {
			// migrate from legacy Void storage key
			encryptedState = this._storageService.get('void.settingsServiceStorageII', StorageScope.APPLICATION)
		}
	...
	private async _storeState() {
		...
		this._storageService.store(TROVE_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: TroveSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: TroveStatefulModelInfo[] = [
...
registerSingleton(ITroveSettingsService, TroveSettingsService, InstantiationType.Eager);
