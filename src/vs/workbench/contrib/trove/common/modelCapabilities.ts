...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, ProviderName } from './troveSettingsTypes.js';
...
export type VoidStaticModelInfo = { // not stateful
	// Trove uses the information below to know how to handle each model.
...
const anthropicModelOptions = {
	'claude-opus-4-8': {
		...
		...anthropicReasoningModelDefaults,
	},
	'claude-opus-4-7': {
		contextWindow: 1_000_000,
		reservedOutputTokenSpace: 8_192,
...
const anthropicSettings: VoidStaticProviderInfo = {
	...
	modelOptionsFallback: (modelName) => {
		...
		// Claude 4.x — match specific versions before generic opus/sonnet patterns
		if (lower.includes('claude-opus-4-8')) fallbackName = 'claude-opus-4-8'
		else if (lower.includes('claude-opus-4-7')) fallbackName = 'claude-opus-4-7'
		...
		else if (lower.includes('claude-opus-4-1')) fallbackName = 'claude-opus-4-5'
		else if (lower.includes('claude-opus-4-0') || lower.includes('claude-opus-4-20250514') || lower.includes('claude-4-opus') || lower.includes('claude-opus-4')) fallbackName = 'claude-opus-4-8'
		...
		else if (lower.includes('claude-sonnet-4-0') || lower.includes('claude-sonnet-4-20250514') || lower.includes('claude-4-sonnet') || lower.includes('claude-sonnet-4')) fallbackName = 'claude-sonnet-4-6'
		else if (lower.includes('claude-haiku-4-5') || lower.includes('claude-haiku-4')) fallbackName = 'claude-haiku-4-5'
...
const xAISettings: VoidStaticProviderInfo = {
	...
	modelOptionsFallback: (modelName) => {
		...
		else if (lower.includes('grok')) fallbackName = 'grok-3'
