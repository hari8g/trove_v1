...
export class TroveMainUpdateService extends Disposable implements ITroveUpdateService {
	...
	async check(explicit: boolean): Promise<TroveCheckUpdateResponse> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Trove to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<TroveCheckUpdateResponse> {
		try {
			const response = await fetch('https://api.github.com/repos/troveeditor/binaries/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Trove is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Trove is up-to-date!'
					}
				}
				else {
