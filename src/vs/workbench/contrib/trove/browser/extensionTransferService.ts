...
const extensionBlacklist = [
	...
	// ignore other AI copilots that could conflict with Trove keybindings
	'sourcegraph.cody-ai',
	'continue.continue',
	'codeium.codeium',
	...
	if (os === 'mac') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Trove', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Trove', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.trove-editor', 'extensions'),
				isExtensions: true,
			}]
	...
	if (os === 'linux') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Trove', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Trove', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.trove-editor', 'extensions'),
				isExtensions: true,
	...
	if (os === 'windows') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Trove', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Trove', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), userprofile, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), userprofile, '.trove-editor', 'extensions'),
				isExtensions: true,
			}]
		} else if (fromEditor === 'Cursor') {
			return [{
