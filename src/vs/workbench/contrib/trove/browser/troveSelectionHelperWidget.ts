...
import { mountTroveSelectionHelper } from './react/out/trove-editor-widgets-tsx/index.js';
...
export type TroveSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	...
	constructor(
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountTroveSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._troveSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies TroveSelectionHelperProps)
