...
import { TROVE_VIEW_CONTAINER_ID, TROVE_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { TROVE_TOGGLE_SETTINGS_ACTION_ID } from './troveSettingsPane.js';
...
const TROVE_OPEN_SIDEBAR_ACTION_ID = 'trove.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: TROVE_OPEN_SIDEBAR_ACTION_ID, title: localize2('troveOpenSidebar', 'Trove: Open Sidebar'), f1: true });
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('troveCmdL', 'Trove: Add Selection to Chat'),
			keybinding: {
...
const TROVE_CMD_SHIFT_L_ACTION_ID = 'trove.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', TROVE_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'trove.settingsAction',
			title: `Trove's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(TROVE_TOGGLE_SETTINGS_ACTION_ID)
	}
})
