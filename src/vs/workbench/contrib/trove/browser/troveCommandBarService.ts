...
import { mountTroveCommandBar } from './react/out/trove-editor-widgets-tsx/index.js'
...
import { TROVE_ACCEPT_DIFF_ACTION_ID, TROVE_REJECT_DIFF_ACTION_ID, TROVE_GOTO_NEXT_DIFF_ACTION_ID, TROVE_GOTO_PREV_DIFF_ACTION_ID, TROVE_GOTO_NEXT_URI_ACTION_ID, TROVE_GOTO_PREV_URI_ACTION_ID, TROVE_ACCEPT_FILE_ACTION_ID, TROVE_REJECT_FILE_ACTION_ID, TROVE_ACCEPT_ALL_DIFFS_ACTION_ID, TROVE_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface ITroveCommandBarService {
...
export const ITroveCommandBarService = createDecorator<ITroveCommandBarService>('TroveCommandBarService');
...
export class TroveCommandBarService extends Disposable implements ITroveCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'trove.TroveCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._troveModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(ITroveCommandBarService, TroveCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type TroveCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountTroveCommandBar(root, accessor, { uri, editor } satisfies TroveCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies TroveCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('troveAcceptDiffAction', 'Trove: Accept Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('troveRejectDiffAction', 'Trove: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('troveGoToNextDiffAction', 'Trove: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('troveGoToPrevDiffAction', 'Trove: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('troveGoToNextUriAction', 'Trove: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('troveGoToPrevUriAction', 'Trove: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('troveAcceptFileAction', 'Trove: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('troveRejectFileAction', 'Trove: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('troveAcceptAllDiffsAction', 'Trove: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: TROVE_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('troveRejectAllDiffsAction', 'Trove: Reject All Diffs in All Files'),
			keybinding: {
