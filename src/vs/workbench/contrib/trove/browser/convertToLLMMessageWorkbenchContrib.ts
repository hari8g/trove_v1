...
class ConvertContribWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.trove.convertcontrib'
	...
	constructor(
		...
		const initializeURI = (uri: URI) => {
			...
			const troveRulesURI = URI.joinPath(uri, '.troverules')
			this.troveModelService.initializeModel(troveRulesURI)
		}
