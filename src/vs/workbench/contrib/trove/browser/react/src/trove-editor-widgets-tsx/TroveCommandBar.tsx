...
import { TroveCommandBarProps } from '../../../troveCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	TROVE_GOTO_NEXT_DIFF_ACTION_ID,
	TROVE_GOTO_PREV_DIFF_ACTION_ID,
	TROVE_GOTO_NEXT_URI_ACTION_ID,
	TROVE_GOTO_PREV_URI_ACTION_ID,
	TROVE_ACCEPT_FILE_ACTION_ID,
	TROVE_REJECT_FILE_ACTION_ID,
	TROVE_ACCEPT_ALL_DIFFS_ACTION_ID,
	TROVE_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const TroveCommandBarMain = ({ uri, editor }: TroveCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@trove-scope ${isDark ? 'dark' : ''}`}
	>
		<TroveCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const TroveCommandBar = ({ uri, editor }: TroveCommandBarProps) => {
	...
	const troveModelService = accessor.get('ITroveModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(TROVE_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(TROVE_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(TROVE_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(TROVE_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(TROVE_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(TROVE_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(TROVE_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(TROVE_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-trove-bg-2 shadow-md border border-trove-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-trove-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-trove-bg-2 rounded shadow-md border border-trove-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-trove-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-trove-bg-2 rounded shadow-md border border-trove-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-trove-border-2 [&>*:last-child]:border-r-0">
