...
import { FeatureName, featureNames, isFeatureNameDisabled, ModelSelection, modelSelectionsEqual, ProviderName, providerNames, SettingsOfProvider } from '../../../../../../../workbench/contrib/trove/common/troveSettingsTypes.js'
import { useSettingsState, useRefreshModelState, useAccessor } from '../util/services.js'
import { _TroveSelectBox, TroveCustomDropdownBox } from '../util/inputs.js'
...
const ModelSelectBox = ({ options, featureName, className }: { options: ModelOption[], featureName: FeatureName, className: string }) => {
	...
	const selection = troveSettingsService.state.modelSelectionOfFeature[featureName]
	const selectedOption = selection ? troveSettingsService.state._modelOptions.find(v => modelSelectionsEqual(v.selection, selection))! : options[0]

	const onChangeOption = useCallback((newOption: ModelOption) => {
		troveSettingsService.setModelSelectionOfFeature(featureName, newOption.selection)
	}, [troveSettingsService, featureName])
...
export const ModelDropdown = ({ featureName, className }: { featureName: FeatureName, className: string }) => {
	...
	const openSettings = () => { commandService.executeCommand(TROVE_OPEN_SETTINGS_ACTION_ID); };
