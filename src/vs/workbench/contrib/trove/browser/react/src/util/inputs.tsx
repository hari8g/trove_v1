	...
	return <>
		...
		{isMenuOpen && (
			<div
				...
				className="z-[100] border-trove-border-3 bg-trove-bg-2-alt border rounded shadow-lg flex flex-col overflow-hidden"
				style={{
					position: strategy,
					top: y ?? 0,
					left: x ?? 0,
			...
			>
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="px-2 py-1 text-trove-fg-1 bg-trove-bg-2-alt border-b border-trove-border-3 sticky top-0 bg-trove-bg-1 z-10 select-none pointer-events-none">
					{optionText ?
				...
				<div className='max-h-[400px] w-full max-w-full overflow-y-auto overflow-x-auto'>
					<div className="w-max min-w-full flex flex-col gap-0 text-nowrap flex-nowrap">
						{options.length === 0 ?
							<div className="text-trove-fg-3 px-3 py-0.5">No results found</div>
							: options.map((o, oIdx) => {
								...
								return (
									...
									<div
										...
										className={`
											...
											${oIdx === optionIdx ? 'bg-blue-500 text-white/80' : 'bg-trove-bg-2-alt text-trove-fg-1'}
										`}
...
export const TroveSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps }: {
	value: string;
	...
	return (
		<input
			...
			disabled={disabled}
			className={`w-full resize-none bg-trove-bg-1 text-trove-fg-1 placeholder:text-trove-fg-3 border border-trove-border-2 focus:border-trove-border-1
...
export const TroveInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline }: {
	onChangeText: (value: string) => void;
	...
	return <WidgetComponent
		className='
			bg-trove-bg-1
			@@trove-force-child-placeholder-trove-fg-1
		'
		ctor={InputBox}
				...
				<div className="relative w-full">
					...
					<div
						className={`relative ${size === 'xxs' ? 'h-0.5' :
							size === 'xs' ? 'h-1' :
								size === 'sm' ? 'h-1.5' :
									size === 'sm+' ? 'h-2' : 'h-2.5'
							} bg-trove-bg-2 rounded-full cursor-pointer`}
						onClick={handleTrackClick}
					>
					...
					<div
						className={`absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2
							...
							bg-trove-fg-1 rounded-full shadow-md ${disabled ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}
							border border-trove-fg-1`}
...
export const TroveCheckBox = ({ label, value, onClick, className }: { label: string, value: boolean, onClick: (checked: boolean) => void, className?: string }) => {
...
export const TroveCustomDropdownBox = <T extends NonNullable<any>>({
	options,
	selectedOption,
	onChangeOption,
	...
	return (
		<div className={`inline-block relative ${className}`}>
			...
			{isOpen && (
				<div
					ref={refs.setFloating}
					className="z-[100] bg-trove-bg-1 border-trove-border-3 border rounded shadow-lg"
					style={{
						position: strategy,
						top: y ?? 0,
						left: x ?? 0,
...
export const _TroveSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className }: {
	...
	return <WidgetComponent
		className={`
			...
			@@[&_select]:!trove-text-trove-fg-3
			@@[&_select]:!trove-text-xs
			!text-trove-fg-3
			${className ?? ''}
		`}
		ctor={SelectBox}
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	return <div ref={divRef} className='relative z-0 px-2 py-1 bg-trove-bg-3'>
		<WidgetComponent
...
export const TroveButtonBgDarken = ({ children, disabled, onClick, className }: { children: React.ReactNode; disabled?: boolean; onClick: () => void; className?: string }) => {
...
// export const TroveScrollableElt = ({ options, children }: { options: ScrollableElementCreationOptions, children: React.ReactNode }) => {
...
// export const TroveSelectBox = <T,>({ onChangeSelection, initVal, selectBoxRef, options }: {
// 	initVal: T;
// 	selectBoxRef: React.MutableRefObject<SelectBox | null>;
// 	options: readonly { text: string, value: T }[];
// 	onChangeSelection: (value: T) => void;
...
// export const TroveCheckBox = ({ onChangeChecked, initVal, label, checkboxRef, }: {
...
const SingleDiffEditor = ({ block, lang, inline = false }: { block: ExtractedSearchReplaceBlock, lang: string | undefined, inline?: boolean }) => {
	const accessor = useAccessor();
	const modelService = accessor.get('IModelService');
	const instantiationService = accessor.get('IInstantiationService');
	const languageService = accessor.get('ILanguageService');
	...
	useEffect(() => {
		...
		const editor = instantiationService.createInstance(
			...
			divRef.current,
			{
				...
				useInlineViewWhenSpaceIsLimited: inline,
				minimap: { enabled: false },
				lineNumbers: inline ? 'on' : 'off',
				lineNumbersMinChars: 3,
				scrollbar: {
					vertical: inline ? 'auto' : 'hidden',
					horizontal: 'auto',
					verticalScrollbarSize: inline ? 8 : 0,
					horizontalScrollbarSize: 8,
					alwaysConsumeMouseWheel: false,
					ignoreHorizontalScrollbarInContentHeight: true,
				},
				...
				renderIndicators: inline,
				padding: inline ? { top: 4, bottom: 4 } : undefined,
			},
		...
		const updateHeight = () => {
			...
			const contentHeight = inline
				? modifiedEditor.getContentHeight() + 8
				: Math.max(
					originalModel.getLineCount() * 19,
					modifiedModel.getLineCount() * 19
				) + 19 * 2 + 1;

			const maxHeight = inline ? 480 : 300;
			const minHeight = inline ? 48 : 100;
		...
		const runUpdateHeight = () => {
			editor.waitForDiff().then(() => updateHeight()).catch(() => updateHeight());
		};
		runUpdateHeight();

		// Update height when content changes
		const disposable1 = originalModel.onDidChangeContent(() => runUpdateHeight());
		const disposable2 = modifiedModel.onDidChangeContent(() => runUpdateHeight());
	...
	}, [originalModel, modifiedModel, instantiationService, inline]);

	return (
		<div className={`w-full ${inline ? 'border-0' : ''} bg-trove-bg-3 @@bg-editor-style-override`} ref={divRef} />
	);
};
...
export const TroveDiffEditor = ({
	...
	contentDiff?: { orig: string, final: string },
	language?: string,
	inline?: boolean,
}) => {
	const accessor = useAccessor();
	...
	if (contentDiff) {
		blocks = [{ orig: contentDiff.orig, final: contentDiff.final, state: 'done' }];
	...
	if (!lang && blocks.length > 0) {
		lang = detectLanguage(languageService, { uri: uri ?? null, fileContents: blocks[0].orig || blocks[0].final });
	}

	// If no blocks, show empty state
	if (blocks.length === 0) {
		return <div className="w-full px-2 py-3 text-trove-fg-4 text-xs">No changes yet</div>;
	}
		...
		<div className="w-full flex flex-col">
			{blocks.map((block, index) => (
				<div key={index} className="w-full">
					{blocks.length > 1 && !inline ? (
						<div className="text-trove-fg-4 text-xs mb-1 px-1">
							Change {index + 1} of {blocks.length}
						</div>
					) : null}
					<SingleDiffEditor block={block} lang={lang} inline={inline} />
				</div>
			))}
		</div>
	);
