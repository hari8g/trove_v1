...
import '../styles.css'
import { TROVE_CTRL_K_ACTION_ID, TROVE_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { Circle, MoreVertical } from 'lucide-react';
import { useEffect, useState } from 'react';

import { TroveSelectionHelperProps } from '../../../../../../contrib/trove/browser/troveSelectionHelperWidget.js';
import { TROVE_OPEN_SETTINGS_ACTION_ID } from '../../../troveSettingsPane.js';


export const TroveSelectionHelperMain = (props: TroveSelectionHelperProps) => {
	...
	>
		<TroveSelectionHelper {...props} />
	</div>
}



const TroveSelectionHelper = ({ rerenderKey }: TroveSelectionHelperProps) => {
	...
	const ctrlLKeybind = keybindingService.lookupKeybinding(TROVE_CTRL_L_ACTION_ID)
	const ctrlKKeybind = keybindingService.lookupKeybinding(TROVE_CTRL_K_ACTION_ID)

	const dividerHTML = <div className='w-[0.5px] bg-trove-border-3'></div>
	...
	useEffect(() => {
		const disposable = commandService.onWillExecuteCommand(e => {
			if (e.commandId === TROVE_CTRL_L_ACTION_ID || e.commandId === TROVE_CTRL_K_ACTION_ID) {
	...
	const defaultHTML = <>
		{ctrlLKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(TROVE_CTRL_L_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
		...
		{ctrlKKeybind &&
			<div
				...
				'
				onClick={() => {
					commandService.executeCommand(TROVE_CTRL_K_ACTION_ID)
					setClickState('clickedOption');
				}}
			>
	...
	const moreOptionsHTML = <>
		<div
			...
			'
			onClick={() => {
				commandService.executeCommand(TROVE_OPEN_SETTINGS_ACTION_ID);
				setClickState('clickedOption');
			}}
		>
			Disable Suggestions?
	...
	return <div className='
		...
		border border-trove-border-3 bg-trove-bg-2
