...
class FilePromptActionService extends Action2 {
	private static readonly TROVE_COPY_FILE_PROMPT_ID = 'trove.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.TROVE_COPY_FILE_PROMPT_ID,
			title: localize2('troveCopyPrompt', 'Trove: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await troveModelService.getModelSafe(uri)).model?.getLanguageId() || '',
