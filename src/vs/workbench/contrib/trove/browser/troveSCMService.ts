...
import { ITroveSCMService } from '../common/troveSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { ITroveSettingsService } from '../common/troveSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/troveSettingsTypes.js'
...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('troveGenerateCommitMessageService');

const loadingContextKey = 'troveSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private voidSCM: ITroveSCMService
	...
	constructor(
		...
		this.voidSCM = ProxyChannel.toService<ITroveSCMService>(mainProcessService.getChannel('trove-channel-scm'))
	}
	...
	async generateCommitMessage() {
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const modelSelection = this.troveSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.troveSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.troveSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'TroveSCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('troveFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'trove.generateCommitMessageAction',
			title: localize2('troveCommitMessagePrompt', 'Trove: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('troveCommitMessagePromptTooltip', 'Trove: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'trove.loadingGenerateCommitMessageAction',
			title: localize2('troveCommitMessagePromptCancel', 'Trove: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('troveCommitMessagePromptCancelTooltip', 'Trove: Cancel Commit Message Generation'),
